...
const AgentSideBar = ({ agentData, jobData, isOpen, onClose }) => {
  ...
  // child-provided updated filterValue + validity
  const [childFilterValue, setChildFilterValue] = useState(null);
  ...
  useEffect(() => {
    ...
    // initialize child data to empty - child will build filterValue from inputValueData
  ...
  const handleChildChange = (filterValue, isValid) => {
    setChildFilterValue((prev) =>
      isEqual(prev, filterValue) ? prev : filterValue
    );
  ...
  // Main run handler that uses childFilterValue
  const handleRun = async () => {
    try {
      ...
      // keeping inputValueData as is and adding filterValue as separate property
      const enrichedData = {
        ...sourceAgent,
